...
function fact(n: number) { let v = 1; for (let i = 2; i <= n; i++) v *= i; return v; }
function poisson(k: number, lambda: number) { return Math.exp(-lambda) * (lambda ** k) / fact(k); }
...
export function predictMatch(homeTeam: string, awayTeam: string, odds?: { home?: number | null; draw?: number | null; away?: number | null }) {
  ...
  const lambdaHome = Math.exp(model.homeAdvantage + atk - defA);
  const lambdaAway = Math.exp(atkA - defH);
  ...
  return {
    ...
    explanation: { homeTeam, awayTeam, usedOddsBlend: Boolean(implied), modelUpdatedAt: model.updatedAt }
  };
}
...
export function evTable(probs: Probs, odds: { home: number; draw: number; away: number }) {
  ...
  const edge = implied ? { H: probs.H - implied.H, D: probs.D - implied.D, A: probs.A - implied.A } : { H: 0, D: 0, A: 0 };
